...
import pymongo
import pandas as pd 
from sqlalchemy import create_engine, Text, String, Interval,Integer,Date
...
# converting DataFrame for channels and migrating into SQL
...
def channeltable(option):
    ...
    # Migrating DataFrame to SQL table
    dfct = dfc.to_sql('channel_table', engine, if_exists='append', index=False, dtype = channel_table)
    return dfct

 

# Converting DataFrame for Videos and migrating to SQL table
...
with st.sidebar:
    ...
    st.write('4.Converting into sql table')
...
if st.button(':violet[Covert to Table]'):
    ...
    # Checking the channel_id in SQL table
    try:
