...
def agg_tran():
    ...
    path="C:/Users/DELL/Desktop/Project2-Phonepay/pulse/data/aggregated/transaction/country/india/state/"
...
def agg_user():
    ...
    #This is to direct the path to get the data as states
    path="C:/Users/DELL/Desktop/Project2-Phonepay/pulse/data/aggregated/user/country/india/state/"
    Agg_state_list=os.listdir(path)
...
def map_tran():
    ...
    path="C:/Users/DELL/Desktop/Project2-Phonepay/pulse/data/map/transaction/hover/country/india/state/"
...
def map_user():
    ...
    path="C:/Users/DELL/Desktop/Project2-Phonepay/pulse/data/map/user/hover/country/india/state/"
    map_state_list=os.listdir(path)
    #map_state_list--> to get the list of states in India
...
def top_tran():
    ...
    path="C:/Users/DELL/Desktop/Project2-Phonepay/pulse/data/top/transaction/country/india/state/"
...
def top_user():
    ...
    path="C:/Users/DELL/Desktop/Project2-Phonepay/pulse/data/top/user/country/india/state/"
    top_state_list=os.listdir(path)
    #top_state_list--> to get the list of states in India
...
def ag_trans():
    ...
    with row2[0]:
        ...
        fig1.update_layout(width=1100, height =650) 
    ...
    with row3[0]:
        ...
        fig2 = px.bar(ag_year, x="state", y="transaction_count",
                      color = "transaction_type", hover_name = "state")
        fig2.update_layout(width=1100, height =650)  
        # Add dropdown menu for year selection on the plot
        st.plotly_chart(fig2)
        
    with row4[0]:
    # Grouped Line Plot
        st.subheader(":green[Transaction Count vs Transaction Amount by Transaction Type and Year]")
        fig1 = px.bar(df1, x='transaction_count', y='transaction_amount', color='transaction_type',
...
def ag_users():
    ...
    with row2[0]:
        st.subheader(":green[User brand Analysis]")
        ...
        fig1 = px.bar(ag_brand, x="state", y="reg_userbrand_count",
                        color="quater", hover_name="state", range_x= [0, 36],
                        )
    ...
    with row4[0]:
        ...
        st.subheader(":green[Statewise User registered Analysis]")
...
def mp_trans():
    ...
    with row1[1]:
        ...
        selected_state = st.selectbox("Select a state", list(states), key="state_selectbox")  # Unique key added
...
# Other parts of your code remain unchange
    ...
    with row2[0]:
        ...
        fig1.update_layout(xaxis_title=selected_state, width=1000, height =500) 
        st.plotly_chart(fig1)
    ...
    with row3[0]:
        ...
        fig2.update_layout(xaxis_title=selected_state, width=1000, height =500)
        st.plotly_chart(fig2)
    ...
    with row5[0]:
        ...
        fig3.update_geos(fitbounds="locations", visible=False)
        st.plotly_chart(fig3, width = 400)
...
def mp_users():
    ...
    with row2[0]:
        ...
        st.subheader(":green[registered users Analysis]")
        mp_user = df4.query('state == @selected_state and year == @selected_year and quater == @selected_qt')
        fig41 = px.bar(mp_user, x =mp_user['district_name'].apply(lambda x: x.replace("district", '')),
                       y = "reg_users",width = 600,color_continuous_scale='Viridis',
            color='reg_users',hover_name="state",
            range_color = (map_users["reg_users"].min(),map_users["reg_users"].min()))
        fig41.update_layout(xaxis_title=selected_state, width=1000, height =500)
        st.plotly_chart(fig41)
        
    with row3[0]:
        st.subheader(":green[user app opens Analysis]")
        fig42 = px.bar(mp_user, x = mp_user['district_name'].apply(lambda x: x.replace("district", '')),
                 y = "reg_users",width = 600,
            color='reg_users',hover_name = "state",color_continuous_scale='Plasma')
        fig42.update_layout(xaxis_title=selected_state, width=1000, height =500)
        st.plotly_chart(fig42)

    with row4[0]:
        column1 = df4["year"].unique()
        selected_yr1 = st.selectbox("Choose a year", column1,key="unique_key1") 
    with row5[0]:
        ag_year1 = df4[df4["year"] == selected_yr1]
        st.subheader(":green[Registered user Analysis]")
        agg_map = ag_year1.groupby(['state'])[['reg_users']].sum()
        agg_map.reset_index(inplace = True)
        ...
        fig3 = px.choropleth(
            ...
            color='reg_users',
            color_continuous_scale='turbo',
            range_color = (agg_map['reg_users'].min(), agg_map['reg_users'].max()),
            )
...
def tp_trans():
    ...
    with row1[0]:
        ...
        if selected_state == 'All states':
            ...
            stateAT = df5.query('state == @selected_state')
            
    
    with row2[0]:
    # Grouped Box Plot
        st.subheader(":green[Transaction Count by the year and quater]")
        fig1 = px.bar(stateAT, x='year', y='transaction_count', color='quater',
                    facet_col='year', facet_col_wrap=3,
                    labels={'transaction_count': 'Transaction Count', 'year': 'Year'})
    ...
    with row3[0]:
    # Grouped Box Plot
        st.subheader(":green[Transaction Amount by the year and quater]")
        fig2 = px.bar(stateAT, x='year', y='transaction_amount', color='quater',
                    facet_col='year', facet_col_wrap=3,
...
def tp_users():
    ...
    with row1[0]:
        ...
        with sub_col1[1]:
            ...
            selected_state = st.selectbox("Select a state", list(states), key="state_selectbox")
        
    with row2[0]:
        st.subheader(":green[Top registered users Analysis]")
        
        mp_tr = df6.query('state == @selected_state and year == @selected_year')
        #mp_tr['pincodes'] = mp_tr['pincodes'].astype(str)
        mp_tr['pincodes'] = mp_tr['pincodes'].astype(str).apply(lambda x: '-'.join([x[:3], x[3:]]) if len(x) > 3 else x)
        fig61 = px.bar(mp_tr, x = "reg_users", y = "pincodes",width = 1000,color_continuous_scale='tempo',
                    color='reg_users',hover_name="state",
...
# Top analysis charts, average analysis charts        
...
def a_trans():
    ...
    fig_2 = px.bar(dfone1, x = 'state', y = 'transaction_count', 
                color_discrete_sequence = px.colors.sequential.Aggrnyl, hover_name = 'state')
    fig_2.update_layout(height = 1700)
...
option = st.sidebar.radio(":blue[Choose your page]", ["Aggregate Analysis", "Map Analysis", "Top Analysis", 
                                                      "Top Analysis charts"])
    
if option == "Aggregate Analysis":
